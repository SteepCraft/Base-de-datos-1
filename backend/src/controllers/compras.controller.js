...
export async function getAllCompras(req, res) {
  try {
    const compras = await models.Compras.findAll();
    res.json(compras);
  } catch (error) {
...
export async function getCompraById(req, res) {
  try {
    const compra = await models.Compras.findByPk(req.params.id);
...
export async function createCompra(req, res) {
  try {
    const compra = await models.Compras.create(req.body);
...
export async function updateCompra(req, res) {
  try {
    const [updated] = await models.Compras.update(req.body, {
...
export async function deleteCompra(req, res) {
  try {
    const deleted = await models.Compras.destroy({
      where: { codi_compra: req.params.id },
    });
    if (!deleted)
      return res.status(404).json({ error: "Compra no encontrada" });
